...
func Generate(database schema.Schema, config Config) ([]byte, error) {
	...
	output.WriteString("var tiqqSchema = tiqq.NewSchemaInfo(tiqq.PostgreSQL)\n\n")
...
func writeTable(output *bytes.Buffer, table schema.Table) {
	...
	fmt.Fprintf(output, "func (table %sTableDef) Insert() postgres.InsertQuery[%s] { return postgres.NewInsert(tiqq.NewInsert[%s, tiqq.PostgreSQLMarker](table.ref, %#v, %#v, %#v)) }\n", typeName, scope, scope, insertableColumns(table.Columns), requiredInsertColumns(table.Columns), uniqueColumns(table))
